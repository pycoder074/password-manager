...
class Entry:
    ...
    class InfoEntry(QWidget):
        def __init__(self, text, *args, **kwargs):
            super().__init__(*args, **kwargs)
            layout = QFormLayout()
            self.entry = QLineEdit(placeholderText = text)
            layout.addRow(self.entry)
            self.setLayout(layout)
    ...
    class PasswordEntry(QWidget):
        def __init__(self, shown=False, verify=False, required=False, *args, **kwargs):
            ...
            text = 'Verify Password:' if verify else 'Password:'
            if required:
                text = '* ' + text

            self.password_box = Entry.InfoEntry(text=text)
